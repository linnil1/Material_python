...
k, T = 1.38E-23, 1.0 # k = Boltzmann constant, and T = temperature in unit K
...
deltav = 10.
vdist = gdisplay(x=800, y=0, ymax = N/5,width=500, height=500, xtitle='v', ytitle='dN')
theory = gcurve(color=color.cyan) # This line and the following 3 are to plot the theoretical speed distribution
dv = 10.
for v in arange(0.,501.+dv,dv):
    theory.plot(pos=(v,(deltav/dv)*N*4.*pi*((m/(2.*pi*k*T))**1.5)*exp((-0.5*m*v**2)/(k*T))*v**2*dv))
observation = ghistogram(bins=arange(0.,500.,deltav),accumulate=1, average=1, color=color.red) # setup for histogram

with open("low.csv","r") as csvFile:
